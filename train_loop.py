...
class Trainer:
    ...
    def train_step(self, batch: Tuple, task_name: str, task_configs: Dict[str, Any]) -> float:
        ...
        if task_name == 'cocktail_party':
            ...
            if x_prime is None or x_prime.numel() == 0: return 0.0
            x_prime, m_star, c_true, l_true = (
                x_prime.to(self.config.device), m_star.to(self.config.device),
                c_true.to(self.config.device), l_true.to(self.config.device)
            )
            # Roles can be None for this task
            if roles:
                roles = {k: v.to(self.config.device) for k, v in roles.items()}
            ...
            for task_name, dataloader in dataloaders.items():
                ...
                for batch_idx, batch in enumerate(dataloader):
                    ...
                    if task_name == 'cocktail_party':
                        ...
                        x_prime, m_star, c_true, l_true, roles = batch
                        if x_prime is None or x_prime.numel() == 0: continue
                        x_prime, m_star, c_true, l_true = (
                            x_prime.to(self.config.device), m_star.to(self.config.device),
                            c_true.to(self.config.device), l_true.to(self.config.device)
                        )
                        if roles: roles = {k: v.to(self.config.device) for k, v in roles.items()}
