...
class Trainer:
    ...
    def train_step(self, batch: Tuple, task_name: str, task_configs: Dict[str, Any]) -> float:
        ...
        if task_name == 'cocktail_party':
            inputs, span_ids, correct_idx = batch
            if inputs.size(0) == 0: return 0.0
            inputs, span_ids, correct_idx = inputs.to(self.config.device), span_ids.to(self.config.device), correct_idx.to(self.config.device)

            if self.config.use_amp and self.config.scaler is not None:
                with torch.amp.autocast('cuda'):
                    scores, loss = self.model(inputs, span_ids=span_ids, correct_idx=correct_idx, task_name=task_name)
            else:
                scores, loss = self.model(inputs, span_ids=span_ids, correct_idx=correct_idx, task_name=task_name)
        elif task_name == 'soft_jigsaw':
            inputs, span_ids, p_star = batch
            if inputs.size(0) == 0: return 0.0
            inputs, span_ids, p_star = inputs.to(self.config.device), span_ids.to(self.config.device), p_star.to(self.config.device)
            ...
            if self.config.use_amp and self.config.scaler is not None:
                with torch.amp.autocast('cuda'):
                    P_hat, loss = self.model(inputs, span_ids=span_ids, p_star=p_star, task_name=task_name, tau=tau)
            else:
                P_hat, loss = self.model(inputs, span_ids=span_ids, p_star=p_star, task_name=task_name, tau=tau)
