...
class Trainer:
    ...
    def train(self, train_loader, val_loader=None):
        ...
        try:
            ...
            print(f"  Train LM Comp: {self.metrics.lm_losses[-1]:.4f}" if self.metrics.lm_losses else "  Train LM Comp: N/A")
            print(f"  Train RankReg Aux: {self.metrics.rank_aux_losses[-1]:.4f}" if self.metrics.rank_aux_losses else "  Train RankReg Aux: N/A")
            print(f"  Train NSP: {self.metrics.nsp_losses[-1]:.4f}" if self.metrics.nsp_losses else "  Train NSP: N/A")
            print(f"  Train SpanSelect: {self.metrics.span_selection_losses[-1]:.4f}" if self.metrics.span_selection_losses else "  Train SpanSelect: N/A")
