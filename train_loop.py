...
class TrainingMetrics:
    def __init__(self):
        ...
        self.rl_rewards = []
        self.final_train_lm_loss = 0.0
        self.final_train_rank_aux_loss = 0.0
        self.final_train_nsp_loss = 0.0
        self.final_train_span_selection_loss = 0.0
    ...
    def update(self, train_loss=None, val_loss=None, learning_rate=None, step_time=None,
        ...
        if lm_loss_component is not None:
            self.lm_losses.append(lm_loss_component)
            self.final_train_lm_loss = lm_loss_component
        if rank_aux_loss is not None:
            self.rank_aux_losses.append(rank_aux_loss)
            self.final_train_rank_aux_loss = rank_aux_loss
        if nsp_loss is not None:
            self.nsp_losses.append(nsp_loss)
            self.final_train_nsp_loss = nsp_loss
        if span_selection_loss is not None:
            self.span_selection_losses.append(span_selection_loss)
            self.final_train_span_selection_loss = span_selection_loss
...
class Trainer:
    ...
    def train(self, train_loader, val_loader=None):
        ...
        try:
            ...
            print(f"  Train LM Comp: {self.metrics.final_train_lm_loss:.4f}")
            print(f"  Train RankReg Aux: {self.metrics.final_train_rank_aux_loss:.4f}")
            print(f"  Train NSP: {self.metrics.final_train_nsp_loss:.4f}")
            print(f"  Train SpanSelect: {self.metrics.final_train_span_selection_loss:.4f}")
