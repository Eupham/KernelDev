...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, roles: Dict[str, torch.Tensor] = None):
        ...
        # Extract role tensors if provided
        ...
        is_mask_marker = roles.get('is_mask_marker') if roles else None
        ...
        out = flash_attention(
            ...
            v=v,
            causal=is_causal,
            attention_mask=None,  # Unused, roles are passed directly
            in_span=in_span,
            span_id=span_id,
            is_prefix=is_prefix,
            is_maskq=is_maskq,
            is_maskmarker=is_mask_marker,
        )
...
class GPTModel(nn.Module):
    ...
    def forward(self, x, targets=None, roles: Dict[str, torch.Tensor] = None, task_name=None, correct_idx=None, p_star=None, tau=0.1, m_star=None, c_true=None, l_true=None):
        ...
        if task_name == 'cocktail_party':
            ...
            # 1) Use MASKQ for the query vector
            maskq_pos = (x == maskq_token_id).nonzero(as_tuple=False)
