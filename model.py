...
class GPTModel(nn.Module):
    ...
    def forward(self, x, targets=None, roles=None, task_name=None, correct_idx=None, p_star=None, tau=0.1, m_star=None, c_true=None, l_true=None, **kwargs):
        ...
        if task_name == 'cocktail_party':
            B, T, D = x_embed.shape
            maskq_id = SPECIAL_TOKENS['[MASKQ]']
            span_id_tok = SPECIAL_TOKENS['[SPAN]']
            es_id = SPECIAL_TOKENS['[ES]']

            maskq_pos = (x == maskq_id).nonzero(as_tuple=False)
            if maskq_pos.numel() == 0:
                return torch.empty(0), torch.tensor(0.0, device=x.device)

            h_q = x_embed[torch.arange(B, device=x.device), maskq_pos[:,1]]

            # gather span means
            span_starts = (x == span_id_tok).nonzero(as_tuple=False)
            span_ends   = (x == es_id).nonzero(as_tuple=False)
            if span_starts.numel() == 0:
                return torch.empty(0), torch.tensor(0.0, device=x.device)

            max_spans = (x == span_id_tok).sum(dim=1).max()
            h_spans = x_embed.new_zeros(B, max_spans, D)

            batch_indices_starts = span_starts[:, 0]
            for i in range(B):
                st_indices = span_starts[batch_indices_starts == i, 1]
                ed_indices = span_ends[batch_indices_starts == i, 1]
