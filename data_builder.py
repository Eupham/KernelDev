    ...
    def _tokenize_text(self, text: str) -> list:
        ...
        while i < len(text):
            ...
            for token_str, token_id in SPECIAL_TOKENS.items():
                ...
                # Emit *all* UTF-8 bytes for this codepoint
                bs = text[i].encode('utf-8')
                tokens.extend([b + NUM_SPECIAL_TOKENS for b in bs])
                i += 1
        return tokens

    def _detokenize_bytes(self, tokens: list, skip_special_tokens=False) -> str:
        special_map = {v: k for k, v in SPECIAL_TOKENS.items()}
        out, buf = [], bytearray()
        for t in tokens:
            if t in special_map:
                if not skip_special_tokens:
                    if buf:
                        out.append(buf.decode('utf-8', errors='strict')); buf.clear()
