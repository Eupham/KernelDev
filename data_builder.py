...
class DataBuilder:
    ...
    def _collate_fn_soft_jigsaw(self, batch):
        ...
        for item in batch:
            ...
            # pad span_id to seq_len if needed
        ...
        attn_masks = torch.stack(batch_attn_masks)        # (B, T, T)
        # expand mask to all heads when we use it in the model
        # we'll do that in the model; for now just return (B, T, T)
