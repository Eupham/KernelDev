...
def _flash_attn_fwd(
    ...
    ATTN_MASK: tl.tensor,
    IN_SPAN: tl.tensor, SPAN_ID: tl.tensor, IS_PREFIX: tl.tensor, IS_CLS: tl.tensor,
    ...
    is_cls_stride_b: int, is_cls_stride_t: int,
    T: int,  #
    TIME_BUCKET:  int,  #
    ...
    for kv_tile_idx in tl.range(
        ...
        if ATTN_MASK is not None:
            ...
            # Load IS_CLS metadata for the key tile
            k_is_cls_ptr = IS_CLS + batch * is_cls_stride_b + kv_indices
            k_is_cls = tl.load(k_is_cls_ptr, mask=kv_indices < seq_len, other=0)

            # Define masks for different regions
            q_is_prefix_b = q_is_prefix[:, None]
            k_is_prefix_b = k_is_prefix[None, :]
            k_is_cls_b = k_is_cls[None, :]

            q_in_span_b = q_in_span[:, None]
            k_in_span_b = k_in_span[None, :]

            q_is_ocean = ~q_is_prefix_b & ~q_in_span_b
            k_is_ocean = ~k_is_prefix_b & ~k_in_span_b

            causal_mask = q_tile_indices[:, None] >= kv_indices[None, :]

            # Rule 1: Prefix queries can attend to prefix keys, causally, and not in spans.
            prefix_attention_mask = q_is_prefix_b & k_is_prefix_b & causal_mask & ~k_in_span_b

            # Rule 2: Non-prefix queries
            # 2a: Can attend to the CLS token
            attend_to_cls_mask = ~q_is_prefix_b & k_is_cls_b

            # 2b: Ocean-to-Ocean attention is causal
            ocean_to_ocean_mask = q_is_ocean & k_is_ocean & causal_mask

            # 2c: Span-to-Ocean attention is bidirectional
            span_to_ocean_mask = q_in_span_b & k_is_ocean

            # 2d: Span-to-Span attention is bidirectional within the same span
            same_span_mask = q_in_span_b & k_in_span_b & (q_span_id[:, None] == k_span_id[None, :])

            # Combine non-prefix query masks
            non_prefix_attention_mask = attend_to_cls_mask | ocean_to_ocean_mask | span_to_ocean_mask | same_span_mask

            mask = prefix_attention_mask | non_prefix_attention_mask
...
def _flash_attn_bwd(
    ...
    if dkv_worker:
        _flash_attn_bwd_dkdv_inner(
            ...
            ATTN_MASK,
            IN_SPAN, SPAN_ID, IS_PREFIX, IS_CLS,
            ...
            is_cls_stride_b, is_cls_stride_t,
            batch=batch,
            head=head,
            tile_id=tile_id,
            seq_len=seq_len,
...
def _flash_attn_bwd_dkdv(
    ...
    for q_tile_idx in tl.range(q_start_tile_idx, q_end_tile_idx, num_stages=PIPELINING):
        ...
        if ATTN_MASK is not None:
            ...
            # Define masks for different regions (transposed view: k is row, q is col)
            q_is_prefix_b = q_is_prefix[None, :]
            k_is_prefix_b = k_is_prefix[:, None]
            k_is_cls_b = k_is_cls[:, None]
...
def attention_forward_adapter(
    ...
    is_cls_token: torch.Tensor = None,
    ...
    fwd_fn[grid](
        ...
        *(strides(is_cls_token, 2) if is_cls_token is not None else [0]*2),
        T=T,
        HEAD_DIM=HEAD_DIM,
        CAUSAL=causal,
...
def attention_backward_adapter_op(ctx, do, dlse):
    q, k, v, o, lse, lens, attention_mask, in_span, span_id, is_prefix, is_cls_token = ctx.saved_tensors
    causal = ctx.causal
...
class IncoherentFlashAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        in_span, span_id, is_prefix, is_cls_token
    ):
...
def flash_attention(
    ...
    is_cls_token: torch.Tensor | None = None,
):
    """
