...
def _flash_attn_fwd(
    ...
    is_prefix_ptr = IS_PREFIX + batch * is_prefix_stride_b + q_tile_indices
    is_prefix_q = tl.load(is_prefix_ptr, mask=q_tile_mask, other=0)

    span_id_ptr = SPAN_ID + batch * span_id_stride_b + q_tile_indices
    span_id_q = tl.load(span_id_ptr, mask=q_tile_mask, other=0)
    ...
    if q_tile_is_prefix:
        ...
        span_end_ptr = SPAN_END + batch * span_end_stride_b + q_tile_indices
    ...
    for kv_tile_idx in tl.range(
        ...
        # Load metadata for K tile
        kv_tile_mask = kv_indices < seq_len
        span_id_k_ptr = SPAN_ID + batch * span_id_stride_b + kv_indices
        span_id_k = tl.load(span_id_k_ptr, mask=kv_tile_mask, other=0)
        is_prefix_k_ptr = IS_PREFIX + batch * is_prefix_stride_b + kv_indices
        is_prefix_k = tl.load(is_prefix_k_ptr, mask=kv_tile_mask, other=0)

        # Create tile-local attention mask from 1D metadata
        # `span_id_q` and `is_prefix_q` are already loaded for the Q tile

        # Base causal mask: k's index is less than or equal to q's index
        causal_mask = q_tile_indices[:, None] >= kv_indices[None, :]

        # Core logic based on span and non-span rules, as per user clarification.
        # A query can attend to a key if:
        # 1. They are in the same span (bidirectional).
        # 2. The key is a non-span token and the attention is causal.
        q_is_span = span_id_q[:, None] != 0
        k_is_nonspan = span_id_k[None, :] == 0
        same_span = q_is_span & (span_id_q[:, None] == span_id_k[None, :])
...
def _flash_attn_bwd_dq(
    ...
    # Recompute KV loop bounds and metadata exactly as in the forward pass
    q_tile_indices = q_token_idx + tl.arange(0, TILE_Q_SIZE)
    q_tile_mask = q_tile_indices < seq_len
    q_len_mask = q_tile_mask[:, None]
    ...
    for kv_tile_idx in tl.range(
        ...
        # Recompute mask exactly as in forward pass
        kv_tile_mask = kv_indices < seq_len
        span_id_k_ptr = SPAN_ID + batch * span_id_stride_b + kv_indices
        span_id_k = tl.load(span_id_k_ptr, mask=kv_tile_mask, other=0)
        is_prefix_k_ptr = IS_PREFIX + batch * is_prefix_stride_b + kv_indices
...
def _flash_attn_bwd_dkdv(
    ...
    # For correctness, we must iterate over all Q tiles and apply the mask,
    # as any Q tile could have attended to the current K tile.
    q_start_tile_idx = 0
    ...
    # Load metadata for the K tile once
    kv_tile_mask = kv_indices < seq_len
    span_id_k_ptr = SPAN_ID + batch * span_id_stride_b + kv_indices
    span_id_k = tl.load(span_id_k_ptr, mask=kv_tile_mask, other=0)
    is_prefix_k_ptr = IS_PREFIX + batch * is_prefix_stride_b + kv_indices
    ...
    for q_tile_idx in tl.range(q_start_tile_idx, q_end_tile_idx, num_stages=PIPELINING):
        ...
        # Recompute mask exactly as in forward pass, but with transposed shape [K, Q]
        q_tile_mask = q_tile_indices < seq_len
        span_id_q_ptr = SPAN_ID + batch * span_id_stride_b + q_tile_indices
        span_id_q = tl.load(span_id_q_ptr, mask=q_tile_mask, other=0)
        is_prefix_q_ptr = IS_PREFIX + batch * is_prefix_stride_b + q_tile_indices
        is_prefix_q = tl.load(is_prefix_q_ptr, mask=q_tile_mask, other=0)
...
def attention_forward_adapter(
    ...
    in_span: torch.Tensor = None, # Kept for API compatibility, but unused
...
def attention_backward_adapter_op(ctx, do, dlse):
    q, k, v, o, lse, lens, attention_mask, in_span, span_id, span_begin, span_end, is_prefix = ctx.saved_tensors
    causal = ctx.causal
    ...
    DQ, DK, DV = torch.ops.flash_attention.backward(
        ...
        in_span=in_span, # Unused, for compatibility
        span_id=span_id,
...
class IncoherentFlashAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        in_span, span_id, span_begin, span_end, is_prefix
    ):
        ...
        ctx.in_span = in_span
        ctx.span_id = span_id
        ctx.span_begin = span_begin
        ctx.span_end = span_end
...
def flash_attention(
    ...
    in_span: torch.Tensor | None = None, # Unused, for compatibility
