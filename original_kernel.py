...
def attention_backward_adapter_op(ctx, do, dlse):
    q, k, v, o, lse, lens, in_span, span_id, is_prefix = ctx.saved_tensors
    causal = ctx.causal
...
class IncoherentFlashAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        O, LSE = torch.ops.flash_attention.forward(
            ...
            return_lse=return_lse or requires_grad,
            precision=precision,
            attention_mask=None,  # Always None, use metadata only
            in_span=in_span,
            span_id=span_id,
            is_prefix=is_prefix,
        )
...
def flash_attention(
    ...
    Args:
        ...
        in_span (Tensor | None): Boolean tensor indicating which tokens are inside spans
        span_id (Tensor | None): Integer tensor indicating span IDs for each token
        is_prefix (Tensor | None): Boolean tensor indicating which tokens are in the prefix
    """
